 ...
 *   3. Systems register via `addSystem`; they run each tick while 'playing'.
 *   4. `engine.destroy()` — unsubscribes from the store, stops the ticker, and
 *      destroys the Pixi application (removing the canvas). Safe to call even if
 *      `init()` has not resolved yet (init()'s await checks the `destroyed`
 *      flag).
 *
 * Entities/systems are NOT created here — they register in PR 6.
 */
export class Engine {
  ...
  async init(host: HTMLElement): Promise<void> {
    ...
    this.onPhaseChange(useGameStore.getState().phase);
  }
  ...
  /** Full teardown — unsubscribe, stop the loop, destroy Pixi. Idempotent. */
  destroy(): void {
    this.destroyed = true;
    if (this.unsubscribe) {
      this.unsubscribe();
